...
(function() {
  const els = document.querySelectorAll(
    '.proj-block, .sk-block, .tag, .exp-card, .cert-card, .edu-row, .btn, .social-icon, .s-tag, .about-stat, .resume-wrap, .proj-desc li, .exp-body ul li'
  );
  els.forEach(el => {
    el.addEventListener('mouseenter', function() {
      this.style.transform = 'translateY(-4px) scale(1.03)';
      this.style.transition = 'transform 0.22s cubic-bezier(.25,.8,.25,1)';
    });
    el.addEventListener('mouseleave', function() {
      this.style.transform = 'translateY(0) scale(1)';
      this.style.transition = 'transform 0.3s cubic-bezier(.25,.8,.25,1)';
    });
  });
})();
/* ── Resume modal ── */
...
(function() {
  ...
  function onScroll() {
    ...
    cubeEl.style.transform = `rotateX(-25deg) rotateY(${45 + scrollY}deg)`;
